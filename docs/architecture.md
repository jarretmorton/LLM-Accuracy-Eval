...
Step 2 — **stability measurement.** For each topic, run the primary query N times (default N=5) at a fixed temperature, and measure agreement across responses. The post's finding: stability after filtering is the strong predictor (R² ≈ 0.995); without filtering, the relationship is much weaker.
...
| **Grader — exact / judge** | String-match and LLM-as-judge graders for non-numeric answers. | Planned |
...
- `grader` — `exact`, `numeric`, `judge`, or `none`, plus `expected_unit` and `refusal_patterns`.
