...
The harness implements both steps as part of the same eval run. A spec that omits the coverage check will still produce stability numbers, but the README and CLI output flag them as unfiltered.
...
flowchart TD
    ...
    P --> C{coverage check<br/>defined?}
    C -->|yes| CC[run secondary questions<br/>web search OFF]
    CC --> F{coverage<br/>passes?}
    C -->|no| R[run primary query × N]
    F -->|yes| R
    F -->|no| SK[skip topic, mark unfiltered]
    R --> G[grader]
    G --> M[compute stability + accuracy metrics]
    M --> O[emit JSONL results]
```
...
| **Driver** | Execute the spec: run coverage checks, run primary queries N times in parallel, manage rate limits. | Stub |
| **Grader** | Score each response. Two grader types planned: `exact` (string match) and `judge` (LLM-as-judge for open-ended answers). | Stub |
| **Metrics** | Compute stability (agreement rate across N runs) and, when ground truth is supplied, accuracy. | Stub |
| **Reporter** | Emit JSONL with one record per query plus a summary record. | Stub |

## Spec format

A spec is a YAML file describing one eval run. The minimum useful spec has:

- `models` — list of model strings to run against (e.g. `claude-opus-4-5-20251101`, `gpt-5-2`).
- `topics` — list of topic objects, each with a `name`, a `primary_query`, optional `coverage_check` (secondary query + expected refusal pattern), and optional `ground_truth`.
- `runs` — N, the number of times to run the primary query per topic per model. Defaults to 20.
- `temperature` — sampling temperature. Defaults to 0.7. Setting this to 0 defeats the point of the harness — stability at temperature 0 is trivially high and uninformative.
- `grader` — `exact`, `judge`, or `none`.
...
- **Concurrency and rate limits.** Naively firing N×M×T requests in parallel will trip every provider's rate limit. The driver needs a token bucket per provider. This is a Week 2 problem at the earliest.
